...
gokalshi implements all **109** Kalshi HTTP endpoints and all **13** WebSocket channels (**23** message types), generated from the pinned spec snapshot in `specs/` (OpenAPI 3.30.0).
...
gokalshi/
  ...
  WebSocket:
    ws_types.go                 ChannelState, WSMessage (SDK infrastructure)
    ws_messages_generated.go    Commands, responses, data types, MsgTypeToChannel (auto-generated from AsyncAPI spec)

  Code Generation:
    generate.go                 //go:generate directives
    tools/generate_types/       OpenAPI → types_generated.go (HTTP request/response types)
    tools/generate_ws_types/    AsyncAPI → ws_messages_generated.go (WS message types)
    tools/sync_godoc/           Sync method godoc from OpenAPI spec
    tools/generate_coverage/    Generate docs/API_COVERAGE.md
```
