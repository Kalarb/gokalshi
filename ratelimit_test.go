...
func TestNewReadWriteTokenBucket(t *testing.T) {
	...
	assert.Equal(t, 0, status.ReadHistoryLen)
	assert.Equal(t, 0, status.WriteHistoryLen)
}
...
func TestRefill_SlidingWindow(t *testing.T) {
	now := 100.0
...
func TestStatus_Snapshot(t *testing.T) {
	...
	assert.Equal(t, 1, status.ReadHistoryLen)
	assert.Equal(t, 1, status.WriteHistoryLen)
}
