...
// Spec: https://docs.kalshi.com/asyncapi.yaml (version 2.0.0, fetched 2026-09-12)
// sha256: dd2089ce4f8f9521d27d137a44adbb1a3aed5290bfe3f9a1f11cd31539672c8c
...
type SubscribeParams struct {
	...
	// cfbenchmarks_value and cfbenchmarks_value_5hz channels only. CF Benchmarks index IDs to seed on t...
	...
	// pyth_value channel only. Underlying tickers to seed on the initial subscribe (omit to subscribe w...
...
const (
	...
	WSErrSubscriptionLimit         = 26
	WSErrTooManyRequests           = 27
	WSErrUnderlyingTickersRequired = 28
)
...
// CfbenchmarksValue5HzData is the msg body for a "cfbenchmarks_value_5hz" message.
type CfbenchmarksValue5HzData struct {
	...
	SourceTSMs int `json:"source_ts_ms"`
	// Index value in USD, formatted with exactly 8 decimal places
	ValueUsd string `json:"value_usd"`
}

// CfbenchmarksValue5HzIndexlistData is the msg body for a "cfbenchmarks_value_5hz_indexlist" message.
type CfbenchmarksValue5HzIndexlistData struct {
...
type MarketLifecycleV2Data struct {
	...
	// Optional - This key will ONLY exist for metadata_updated events. The cap (upper bound) strike val...
	CapStrike float64 `json:"cap_strike,omitempty"`
	...
	// Optional - This key will ONLY exist for metadata_updated events with a custom or structured strik...
	CustomStrike map[string]any `json:"custom_strike,omitempty"`
	...
	// Optional - This key will ONLY exist when the market is created. Identifier for the exchange shard...
	ExchangeIndex int `json:"exchange_index,omitempty"`
	// Optional - This key will ONLY exist for metadata_updated events. The floor (lower bound) strike v...
	FloorStrike float64 `json:"floor_strike,omitempty"`
	...
	// Optional - Emitted alongside price_level_structure (on market creation and price_level_structure_...
	PriceRanges []map[string]any `json:"price_ranges,omitempty"`
	...
	// Optional - This key will ONLY exist for metadata_updated events. Determines how floor_strike / ca...
	StrikeType string `json:"strike_type,omitempty"`
...
// MultivariateMarketLifecycleData is the msg body for a "multivariate_market_lifecycle" message.
type MultivariateMarketLifecycleData struct {
	// Optional - This key will be emitted when the market is created
	...
	// Optional - This key will ONLY exist when the market is created. Unix timestamp for when the marke...
	OpenTS int64 `json:"open_ts,omitempty"`
	// Optional - This key will exist when the market is created. The price level structure of the market
	PriceLevelStructure string `json:"price_level_structure,omitempty"`
	// Optional - This key will ONLY exist when the market is determined. Result of the market
	Result string `json:"result,omitempty"`
	// Optional - This key will ONLY exist when the market is settled. Unix timestamp for when the marke...
...
// PythValueData is the msg body for a "pyth_value" message.
type PythValueData struct {
...
// PythValueUnderlyingListData is the msg body for a "pyth_value_underlying_list" message.
type PythValueUnderlyingListData struct {
...
type RFQCreatedData struct {
	...
	MarketTicker string `json:"market_ticker"`
	// Multivariate event collection ticker. Omitted for non-MVE RFQs.
	MVECollectionTicker string `json:"mve_collection_ticker,omitempty"`
	// Selected legs for an MVE RFQ. Omitted when the array is empty.
